...
export default function MyMealsListScreen() {
  ...
  const handleDeleteMyMeal = async (meal: MyMeal) => {
    console.log(`[MyMealsList] 🗑️ DELETE STARTED - Meal: "${meal.name}" (ID: ${meal.id})`);
    ...
    try {
      ...
      console.log(`[MyMealsList] 🎯 Removing from local state`);
      setMyMeals(prevMeals => {
        const filtered = prevMeals.filter(m => m.id !== mealId);
        console.log(`[MyMealsList] 📊 Local state updated: ${prevMeals.length} → ${filtered.length} meals`);
        return filtered;
      });

      // STEP 2: Delete meal items first (foreign key constraint)
      console.log(`[MyMealsList] 🗑️ Deleting meal items from backend...`);
      ...
      console.log(`[MyMealsList] ✅ Meal items deleted successfully`);

      // STEP 3: Delete the meal
      console.log(`[MyMealsList] 🗑️ Deleting meal from backend...`);
      ...
      console.log(`[MyMealsList] ✅ Meal deleted from backend successfully`);
      ...
      console.log(`[MyMealsList] 🎉 DELETE COMPLETE - "${mealName}" deleted successfully`);
    } catch (error) {
      console.error(`[MyMealsList] ❌ Unexpected error:`, error);
