...
export default function ProfileScreen() {
  ...
  useFocusEffect(
    useCallback(() => {
      console.log('[Profile iOS] Screen focused, loading data and syncing subscription');
      loadUserData();
      // Sync subscription from Apple IAP when screen is focused
      syncSubscription();
  ...
  const loadUserData = async () => {
    try {
      setLoading(true);
      console.log('[Profile iOS] Starting to load user data...');
      ...
      if (userError) {
        ...
        console.log('[Profile iOS] User data loaded successfully');
      ...
      if (goalError) {
        ...
        console.log('[Profile iOS] Active goal loaded successfully');
        setGoal(activeGoal);
      } else {
      ...
      console.log('[Profile iOS] ✅ Data loading complete');
    } catch (error) {
  ...
  const onRefresh = async () => {
    console.log('[Profile iOS] Refreshing data...');
    setRefreshing(true);
    await Promise.all([loadUserData(), syncSubscription()]);
  };

  const handleManageSubscription = async () => {
    console.log('[Profile iOS] Navigating to paywall');
    router.push('/paywall');
  };
    ...
    Alert.alert(
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          style: 'destructive',
          onPress: async () => {
            console.log('[Profile iOS] User logging out');
  ...
  const openStartDatePicker = () => {
    ...
    if (goal.start_date) {
      ...
      console.log('[Profile iOS] Opening date picker with stored date:', goal.start_date);
  ...
  const getSubscriptionStatusText = () => {
    ...
    console.log('[Profile iOS] Subscription status check:', {
    ...
    if (subscription.status === 'active' || subscription.status === 'trialing') {
      console.log('[Profile iOS] ✅ Status: Active (subscription is active or trialing)');
      return 'Active';
    }

    // Canceled but still within paid period = show "Active (Canceled)"
    if (subscription.status === 'canceled' && periodEnd && periodEnd > now) {
      console.log('[Profile iOS] ⚠️ Status: Active (Canceled) - subscription canceled but still active until', periodEnd.toISOString());
      return 'Active (Canceled)';
    }

    // Canceled and period expired, or any other status = show "Free"
    console.log('[Profile iOS] ❌ Status: Free (subscription canceled and expired, or other status)');
    return 'Free';
  };
