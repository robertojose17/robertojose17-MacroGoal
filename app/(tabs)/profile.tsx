...
export default function ProfileScreen() {
  ...
  const loadUserData = useCallback(async () => {
    try {
      ...
      if (!authUser) {
        console.log('ProfileScreen: No authenticated user');
        Alert.alert('Error', 'Not authenticated');
        return;
      }

      console.log('ProfileScreen: Loading user data for:', authUser.id);
      ...
      if (userError) {
        console.error('ProfileScreen: Error loading user data:', userError);
        throw userError;
      }
      ...
      if (goalError && goalError.code !== 'PGRST116') {
        console.error('ProfileScreen: Error loading goal data:', goalError);
        throw goalError;
      }

      console.log('ProfileScreen: Goal data loaded:', goalData);

      setUser(userData);
      setGoal(goalData || null);

      logSubscriptionStatus('ProfileScreen', {
        isSubscribed,
        subscriptionStatus,
        subscriptionLoading,
      });
    } catch (error: any) {
      console.error('ProfileScreen: Error in loadUserData:', error);
  ...
  useFocusEffect(
    useCallback(() => {
      console.log('ProfileScreen: Screen focused, loading data');
      loadUserData();
  ...
  const handleManageSubscription = useCallback(() => {
    console.log('ProfileScreen: Navigating to subscription management');
  ...
  const handleEditGoals = useCallback(() => {
    console.log('ProfileScreen: Navigating to edit goals');
  ...
  console.log('ProfileScreen: Rendering, loading:', loading, 'user:', !!user);
